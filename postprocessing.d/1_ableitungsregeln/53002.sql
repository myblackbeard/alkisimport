...
FROM ax_strassenverkehrsanlage o
LEFT OUTER JOIN ap_ppo_unnested p ON o.gml_id = p.dientzurdarstellungvon_unnested AND p.art='BEZ' AND p.endet IS NULL
LEFT OUTER JOIN ap_darstellung_unnested d ON o.gml_id = d.dientzurdarstellungvon_unnested AND d.art='BEZ' AND d.endet IS NULL
...
FROM ax_strassenverkehrsanlage o
LEFT OUTER JOIN ap_pto_unnested t ON o.gml_id = t.dientzurdarstellungvon_unnested AND t.art='BEZ_TEXT' AND t.endet IS NULL
LEFT OUTER JOIN ap_darstellung_unnested d ON o.gml_id = d.dientzurdarstellungvon_unnested AND d.art='BEZ_TEXT' AND d.endet IS NULL
WHERE o.endet IS NULL AND NOT bezeichnung IS NULL;
...
FROM (
	SELECT
		...
		coalesce(t.advstandardmodell||t.sonstigesmodell,o.advstandardmodell||o.sonstigesmodell) AS modell
	FROM ax_strassenverkehrsanlage o
	LEFT OUTER JOIN ap_pto_unnested t ON o.gml_id = t.dientzurdarstellungvon_unnested AND t.art='ART' AND t.endet IS NULL
