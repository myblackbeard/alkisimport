...
BEGIN
	...
	LOOP
		...
		SELECT st_linemerge(st_collect(wkb_geometry)) INTO uk FROM ax_gelaendekante_unnested WHERE r0.gml_id = istteilvon_unnested AND artdergelaendekante=1230 AND endet IS NULL;

		-- RAISE NOTICE 'Unterkante:%', st_astext(uk);

		-- Alle Kanten sind Schnittkanten
		SELECT st_union(wkb_geometry) INTO sk FROM ax_gelaendekante_unnested WHERE r0.gml_id = istteilvon_unnested AND endet IS NULL;
		IF sk IS NULL THEN
			kskn := kskn + 1;
		...
		FOR r1 IN
			...
			FROM ax_gelaendekante_unnested
			WHERE r0.gml_id = istteilvon_unnested
